...
def _order_totals(context):
    ...
    """
    if "order" in context:
        for f in ("item_total", "shipping_total", "discount_total", "bundle_discount_total"):
        ...
        if context["item_total"] == 0:
            ...
            for f in ("shipping_type", "shipping_total", "discount_total", "bundle_discount_total"):
    ...
    if context.get("bundle_discount_total", None) is not None:
        context["order_total"] -= context["bundle_discount_total"]
    return context
...
def related_products_by_keywords(context, product):
    ...
    products = Product.objects.published().filter(in_stock=True).exclude(pk=product.pk).filter(keywords__keyword__in=keyword_ids, master_item_code__startswith=product.master_item_code[:2]).distinct()
    context.update({
